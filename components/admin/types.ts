...
// Liegt in lib/, damit auch Nicht-Komponenten sie nutzen können (etwa
// lib/send-timeline.ts). Hier re-exportiert, weil sehr viele Komponenten sie
// aus dieser Datei importieren.
import { parseDbDate } from '@/lib/parse-db-date'
export { parseDbDate }

export function formatDate(dateStr: string): string {
  return parseDbDate(dateStr).toLocaleString('de-CH', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
    hour: '2-digit',
...
export function formatDateShort(dateStr: string): string {
  return parseDbDate(dateStr).toLocaleDateString('de-CH', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
  })
